...
6. **GPU as a first-class platform resource.** `compression/gpu_batcher.py` accumulates GPU-bound work (ALETHEIA, ANAMNESIS, quality scoring) and batches against the configured `GPU_PROVIDER_HOST` endpoint. Each engine declares `gpu_intent` (`cpu_only` | `gpu_optional` | `gpu_required`). Endpoint is backend-agnostic — Ollama on an Intel iGPU, vLLM on an A10, a remote provider. CPU fallback is mandatory for every `gpu_optional` engine; `gpu_required` engines are skipped with `reject_reason='disabled'` when the endpoint is unreachable.
